			...
			client.query(stmt, stmtArr, function(err, results){
				...
				if(err){
					if(parseInt(err.code) === 23505){
						callback("sorry, "+newuser.email.toLowerCase() + " has already been taken.");
						return console.error('err', err);
					}
					else{
						callback("internal database error");
				...
				if(results.rows){
					...
					callback("Sorry, that Email has already been taken.");
					return;
				}
			});
		});
		...
		pg.connect(conString, function(err, client, done){
			...
			client.query(stmt, stmtArr, function(err, result){
				...
				if(result.rows[0]){
					...
					var img = { large: result.imagename, small: result.imagename };
...
function recipeToYummly(rec){
	...
	ob.uid = rec.uid;
	// var img = getRandomImage();
	var img = { large: rec.imagename, small: rec.imagename };
