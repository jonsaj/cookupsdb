...
function getRandomImage(){
	...
	*/
	...
	// nutrition.ATTR_NAME.{min|max}
		...
		/*
		* if we have db total matches, we only need our returning
		* 	number of results.
		*/
		if(query.dbtotalMatchCount){
...
/*
			 	if(result && result.rows[0]){callback(err, resultToYummly(result.rows));}
				else{callback("No results found");}
*/
				else{
					var yumres = resultToYummly(result.rows);
					if(! query.dbtotalMatchCount){
						query.dbtotalMatchCount = result.rows.size;
					}
					callback(err, yumres, query.dbtotalMatchCount);
				}
			});
		});
		pg.end();
...
/*
	convert a db recipe to yummly format
*/
...
/*
	neatly put responses into an array for yummly merge
*/
