...
function getRandomImage(){
	...
	function login(user, callback){
		...
		var stmt = "SELECT uid, email, uname, password, picture FROM users WHERE email = LOWER($1)";
		stmt += ";";
	...
	function addUser(newuser, callback){
		...
		var stmt = "INSERT into USERS values(DEFAULT,$1,$LOWER($2),$3";
	...
	*/
	...
	function editUser(user, callback){
		...
		if(user.email) {
			...
			stmtArr.push(user.email); stmt += "email = $LOWER($" + stmtArr.length + ")" ;
		}
		if(user.password) {
			if(stmtArr.length > 0) { stmt += "," ; }
